...
public class FallenAasimarUtils {
    ...
    public List<CharRaceAbility> formAbilities(Character character) {
        ...
        Optional<Ability> abilityPresent4 = Optional.ofNullable(abilityRepo.findByName("САВАН СМЕРТИ"));
            ...
            if (abilitiesList.get(i).isEmpty()) {
                switch (i) {
                    ...
                    case 2 -> ability = Ability.builder()
                            .name("НЕСУЩИЙ СВЕТ")
                            .description("Вам известен заговор cвет. Базовой характеристикой для его использования является Харизма.")
                            .isActive(false)
                            .requiresRest(false)
                            .typeOfRest(RestEnum.NONE)
                            .abilitySource(AbilityEnum.RACE)
                    ...
                    case 3 -> {
                        if (CharacterCalculator.getLevel(character.getExperience()) >= 3) {
                            ability = Ability.builder()
                                    .name("САВАН СМЕРТИ")
                                    .description("""
                                            Вы можете действием высвободить божественную энергию внутри себя, \
                                            заставляя ваши глаза превратиться в бездну тьмы, а два призрачных, не \
                                            способных взлететь скелета крыльев вырасти у вас за спиной.
                                            В момент вашего превращения, другие существа в пределах 10 футов, которые \
                                            могут вас видеть, должны преуспеть в спасброске Харизмы (Сл 8 + ваш бонус \
                                            мастерства + ваш модификатор Харизмы) или станут испуганными вами до конца \
                                            вашего следующего хода. Ваше превращение длится 1 минуту или пока вы не \
                                            закончите его бонусным действием.
                                            Во время превращения раз в свой ход вы можете нанести дополнительный урон \
                                            некротической энергией одной цели, когда вы наносите ей урон атакой или заклинанием. \
                                            Дополнительный урон некротической энергией равен вашему уровню.
